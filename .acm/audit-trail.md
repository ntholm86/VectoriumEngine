...
## 2026-07-06 - neon-glow-shapes-and-gravity-collision-leak-fix

**Tier: Standard-to-heavy (operator pushback + a real engine defect found).** Operator rejected the earlier "no bug, by design" conclusion on sprites twice ("it does not look correct - its just squares, triangles, shapes"). Asked a clarifying question (sprites vs polished shapes); operator was unavailable ("work autonomously"). Re-confirmed no image assets exist anywhere in the repo and no image-generation tool is available, so fabricating "sprite art" would just be worse placeholder blobs. Proceeded on best judgment: improve the existing SDF shapes instead.

**[!DECISION]** Added a neon rim-glow + outward halo to the SDF fragment shader composition, in BOTH InstancedShapeRenderer.ts (flagship instanced path) and WebGLBatchRenderer.ts (legacy WebGL1 fallback), keeping visual parity between them. Purely color/alpha math on the already-computed SDF distance -- no geometry or vertex-format changes, so no risk to the benchmarked instanced path. First pass was too subtle to register as a visible change; strengthened to a combined inner-rim brighten + outward halo. Verified visually across ONSLAUGHT (clear glowing edges on terrain/enemies/player) after a full rebuild.

**[!REALIZATION] A second, unrelated, much bigger finding surfaced while scoping a new "physics-driven shots" game request:** setGravityEnabled(id, true)/setCollisionsEnabled(id, true) are documented and JS-implemented as PER-ENTITY toggles, but the WASM physics kernel (assembly/physics.ts) never received those per-entity flag arrays at all -- applyGravity() and detectCollisionsInternal() only checked sleepState[i], gated by an aggregate "if (gravityCount > 0)" counter, then applied to EVERY awake entity in the world. Verified live through the real WASM path (not the JS fallback): two identically-created entities, gravity enabled on only one -- both fell identically before the fix (vy=300 after 30 frames), confirming the leak. This was never noticed because no existing game-portal scene calls these setters at all (all do manual physics), so the WASM gravity/collision phases have simply never run in production until now.

**[!DECISION] Fixed properly rather than working around it in the new game,** since "demonstrates the physics" implies the real engine system, not a manual reimplementation. Added enableGravityFlags/enableCollisionsFlags as new WASM-owned zero-copy arrays (assembly/physics.ts), mirroring the exact existing pattern for animationType/fadeDirection etc. (new pointer exports, wired through WasmPhysicsBridge.ts as new views, swapped into World.ts with old-data preservation on the swap). applyGravity() now checks the per-entity flag; detectCollisionsInternal() requires BOTH entities in a candidate pair to have collision enabled (SIMD path: precomputed per-lane flag mask ANDed into the existing bitmask; scalar path: early continue).

**Verified post-fix, live, through the real WASM path (not JS fallback):** gravity isolation confirmed (flagged entity falls, unflagged stays at rest); collision isolation confirmed (flagged pair visibly interacts and stops at touching distance, unflagged pair passes straight through unaffected). Full build clean, 32/32 tests green. Rebuilt physics.wasm + dist/vectorium.js; game-portal's vectorium-engine dependency is a directory junction to this repo so the fix was immediately visible there too (after copying the rebuilt .wasm binaries, which aren't part of the junction/symlink).

**Blind spot, named not fixed:** applyBoundaryConstraints() (world-edge bounce) still applies unconditionally to every entity regardless of any flag -- a separate, lower-impact leak (world bounds are typically far outside normal play) not addressed this run. Also: the collision-enabled test pair did not show a clean elastic bounce-apart (velocities stayed unchanged even after position correction froze them at touching distance) -- likely a pre-existing resolveCollisionsInternal nuance (mass defaults, head-on symmetric approach) unrelated to the per-entity gating fix, not chased further given it was outside this session's scope.

Committed as 1955232, pushed to origin/back-to-scratch.

Cost: heavy - read/edited assembly (WASM) + 2 JS bridge/core files + 2 shader files, 2 WASM rebuilds, 1 full build+test run, ~8 live verification rounds across 3 fresh page loads (needed to rule out browser/HTTP caching of the old .wasm binary), 1 commit+push. No subagent.
